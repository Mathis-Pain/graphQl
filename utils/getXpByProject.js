  ...
  return `
    <svg width="${svgWidth}" height="${svgHeight}" viewBox="0 0 ${svgWidth} ${svgHeight}" style="overflow: visible;">
      ...
      <g font-family="Arial" font-size="10" fill="white">
        <!-- Axe Y (vertical) -->
        <line x1="30" y1="20" x2="30" y2="${svgHeight - 50}" stroke="white"></line>
        <!-- Axe X (horizontal) -->
        <line x1="30" y1="${svgHeight - 50}" x2="${svgWidth - 20}" y2="${svgHeight - 50}" stroke="white"></line>
        <!-- Barres -->
        ${bars}
        <!-- Labels -->
        <text x="15" y="15" fill="white">XP (k)</text>
